"""Full WA enrichment run (Step 6): all 191 filings, grouped by target company.

Loads the search-only workbook (`output/wa_all_companies_search.xlsx`),
hydrates Filing objects, then runs detail-page enrichment group-by-group so we
can save intermediate results between each company. Rate/Rule filings get PDF
download + parse; Form and Scoring Model filings get metadata only
(`enrich_filing` already branches on `filing_type == "Rate/Rule"`).

Missing `submission_date`s from Step 4b are backfilled automatically by
`scrape_detail_fields`.

Final output: `output/wa_final.xlsx` plus a printed summary of the 6 metrics
requested by the spec (parseable rate effects, premium-neutral, unparseable,
download failures, total PDF bytes, runtime).
"""
...
SEARCH_XLSX = OUTPUT_DIR / "wa_all_companies_search.xlsx"
FINAL_XLSX = OUTPUT_DIR / "wa_final.xlsx"
...
def _summarize(filings: list[Filing], elapsed_s: float) -> None:
    ...
    print("\n" + "=" * 60)
    print(f"WA FULL RUN SUMMARY ({len(filings)} filings)")
    print("=" * 60)
...
def main() -> int:
    ...
    started = time.time()
    group_order = sorted(groups.keys(), key=lambda k: len(groups[k]))  # small groups first
    ...
    with sync_playwright() as pw:
        ...
        try:
            for gi, (state, company) in enumerate(group_order, 1):
                ...
                # Intermediate save after each group completes.
