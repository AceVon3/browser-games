"""Full ID enrichment run (Step 7): all target-company filings in Idaho.

Mirrors `run_wa_full.py` with ID-specific paths. Loads the search-only workbook
(`output/id_all_companies_search.xlsx`), hydrates Filing objects, then runs
...
Final output: `output/id_final.xlsx` plus a printed summary.
"""
...
SEARCH_XLSX = OUTPUT_DIR / "id_all_companies_search.xlsx"
FINAL_XLSX = OUTPUT_DIR / "id_final.xlsx"
DEFAULT_STATE = "ID"
...
def _summarize(filings: list[Filing], elapsed_s: float) -> None:
    ...
    print("\n" + "=" * 60)
    print(f"ID FULL RUN SUMMARY ({len(filings)} filings)")
    print("=" * 60)
