"""Full OR enrichment run: all target-company filings in Oregon.

Mirrors `run_id_full.py` with OR-specific paths. Loads the search-only workbook
(`output/or_all_companies_search.xlsx`), hydrates Filing objects, then runs
...
Final output: `output/or_final.xlsx` plus a printed summary.
"""
...
SEARCH_XLSX = OUTPUT_DIR / "or_all_companies_search.xlsx"
FINAL_XLSX = OUTPUT_DIR / "or_final.xlsx"
DEFAULT_STATE = "OR"
...
def _summarize(filings: list[Filing], elapsed_s: float) -> None:
    ...
    print("\n" + "=" * 60)
    print(f"OR FULL RUN SUMMARY ({len(filings)} filings)")
    print("=" * 60)
